...
**Canonical vectors**: Run `python3 scripts/novaseal_canonical_vectors.py --pretty` after schema-layout extraction to produce `target/novaseal-canonical-vectors.json`, deterministic packed-reference test bytes for the eight fixtures.
...
│   ├── COMBINED_TX_HARNESS.md # eight-fixture lock+type ckb-script verifier
...
4. Preserve the current combined local CKB transaction verifier layer and live local devnet stateful runner while adding public/shared CellDep attestation and external BIP340 TCB review.
