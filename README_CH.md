...
`receipt`、`action`、`lock`、`consume`、`create`、`read`、
...
lock owner_only(protected wallet: Wallet, witness signer: Address) -> bool {
...
编译器把 `consume`、`create`、`transfer`、`destroy`、`claim`、`settle`、
action 边界的 `read` 参数，以及表达式层的 `read_ref<T>()` 当作
**Cell effect**，而不是普通函数调用。这些 effect 会反映到
