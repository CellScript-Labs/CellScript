...
- **v0.23** — *Can the compiler ship as running infrastructure and absorb
  ...
  enforce the completed native test/fixture tooling boundary, close the next slice of
...
| v0.23 planned scope | Public registry production deployment on `cellscript.dev`, completed native test/fixture tooling with repository-wide source-policy enforcement, deeper RGB++ / Fiber integration, and an Off-Chain Session Runtime profile with initial concurrency support so the Myelin vendored fork can re-converge on upstream. | [v0.23 roadmap](CELLSCRIPT_0_23_ROADMAP.md) |
