...
Version line: the workspace `Cargo.toml` pins `version = "0.30.0"`, Rust
...
The 0.30 package closure is lock-authoritative. `Cell.lock` version 5 uses
`cellscript-lock-v0.30-single-package-coordinate-v1`, enforces the
`single-package-coordinate-v1` resolver model, and binds the root manifest
