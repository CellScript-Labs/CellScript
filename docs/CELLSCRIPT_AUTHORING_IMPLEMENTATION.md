...
| Remaining business corpus | Exact per-row fixtures for the frozen NFT/DOB, fungible, committed-state, temporal, authorization, multi-Script, external-verifier, and order/AMM scenarios. | NFT/DOB is complete with a signed persistent lifecycle and a separate exact capacity typed-view artifact; external-verifier is complete with exact EXEC, SPAWN/WAIT, and verifier-handle transactions; fungible, committed-state, temporal, authorization, and multi-Script are also exact. Four order/AMM rows remain pending. |
