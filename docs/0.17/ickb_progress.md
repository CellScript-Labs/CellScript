...
- 本次工作范围：Witness/Molecule/Auth 解析闭环、0.18 一等公民 Script API、arbitrary fixed-byte Script construction、DAO withdrawal capacity-compensation runtime formula evidence、DAO witness input_type 差分拒绝证据
- 忽略未跟踪目录：`typescript`
...
一等公民 `Script` API 不属于 0.17 交付范围；它已进入 0.18 实现轨道。
0.17 只保留 helper-level Script support；0.18 已跨到 arbitrary fixed-byte
Script construction，并用真实 CKB VM fixture 验证构造值和现有 lock/type Script 的匹配。
...
| 一等公民 `Script` API | 0.18 已进入实现 | 0.17 只保留 helper-level Script support；0.18 已实现 `ScriptRef` 读取、`ScriptArgs` fixed-byte 构造、`Script` 构造、`script::require_cell_lock_matches/type_matches`，并新增 canonical `ckb-types::packed::Script` 字节/哈希对齐测试和 ckb-testtool type ScriptGroup continuation pass/reject fixture。 | 继续把 iCKB 需要的具体 Script 形状迁入差分矩阵；deploy manifest resolution、CellDep solving、TYPE_ID constructor policy、任意 in-script Script hash synthesis仍留给后续 builder/manifest 阶段。 |
...
## Roadmap decision: first-class Script API entered 0.18
...
lock/type pair scan 已支撑现有 76 条差分行与 active `MODEL` 清零。

0.18 不再停留在 read-only ScriptRef。当前实现已经跨到 arbitrary fixed-byte
Script construction：

- `script::args(b"...")` / `script::args(hash)` 构造 `ScriptArgs`
- `script::new(code_hash, hash_type, args)` 构造一等 `Script`
- `expected.code_hash`、`expected.hash_type`、`expected.args.len` 可被 verifier 继续引用
- `script::require_cell_lock_matches(source, expected)`
- `script::require_cell_type_matches(source, expected)`
- off-chain `CkbScriptValue::packed_bytes()` / `hash()` 与 `ckb-types` canonical packed Script 对齐

本轮 `tests/v0_18.rs` 新增了真实 CKB VM 证据：同一个 type script 的
input/output continuation 中，CellScript 构造的 expected Script 会读取 input
lock Script，匹配 `b"owner"` 时通过，改成 `b"wrong"` 时拒绝。这个 fixture 同时
暴露并修复了三个 VM-only codegen 问题：非空 args prefix read 的 table-size
校验、运行时加载 Hash 的 buffer metadata alias、exact args helper stack frame
覆盖 SourceView state。

0.18 仍不声称完整 deploy/builder 层：

- deployment manifest resolution
- CellDep solving
- TYPE_ID constructor policy
- non-TYPE-ID global uniqueness proof
- arbitrary in-script Script hash synthesis as a DSL builtin
- Action Builder / CCC transaction generation

中文口径：0.17 先补选定 iCKB 差分证据；0.18 把 Script 从 helper 收束层推进到
一等构造/匹配能力，但部署解析、交易生成和全局唯一性证明仍属于后续阶段。
...
### 0.18 First-Class Script API scope
...
- `script::args(...)` fixed-byte args construction
- `script::new(code_hash, hash_type, args)` first-class Script construction
- `script::require_cell_lock_matches(source, expected)`
- `script::require_cell_type_matches(source, expected)`
- canonical off-chain `CkbScriptValue` packed bytes/hash
...
该项已从只读 ScriptRef 推进到 arbitrary fixed-byte Script construction。
当前 0.17 进度文档只记录它对 iCKB 等价工作的解锁意义；后续仍要把具体 iCKB
Script 形状迁入 original-vs-CellScript 差分矩阵。
