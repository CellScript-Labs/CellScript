...
- 最新已提交基线：`6359e4a feat(witness): add feature flags, tests, docs and clippy fix`
- 本次提交范围：Witness P1 runtime ABI/Molecule/Hash bridge 修复、proof-plan witness reads 修复，以及真实 CKB VM witness 回归测试
- 忽略未跟踪目录：`typescript`
...
`remaining_model_blockers` 为空。`non_executable_model_assumptions` 现在登记
3 个 legacy model 假设：`duplicate receipt` 的 receipt-id double-mint、
...
同时 production gate 已加固：只要 `non_executable_model_assumptions` 非空，
未来任何 `PROVEN` / `EXECUTED_CKB_VM_DIFF` 声明都会被拒绝。
...
本次新增三条协议中立的 CellScript-only CKB VM witness 回归测试，不新增
...
Hash。该组测试实际走 `LOAD_WITNESS` 和 `ckb-testtool` 交易上下文，比单纯
...
仍为 `NOT_PROVEN`，必须等 witness/auth、一等 `Script`、通用聚合降低、完整
...
| Witness/Molecule/Auth 解析 | 55-60% | 已实现 `witness::size`（U64，标量 fail-closed 约定）、`witness::raw`（Hash，带 caller buffer 桥接）和 `ckb::require_witness_size_at_least`（void runtime requirement，正确保存 `min_size` 与栈帧管理）；已实现完整 WitnessArgs Molecule 表头解析（16 字节头：total_size + 3 offsets，field_count 从 offset0/4-1 推导）与 BytesOpt 字段提取（lock/input_type/output_type，带 caller buffer 写入、None 支持、短字段零填充）；Hash 返回路径已加入 `emit_runtime_witness_hash_call` 桥接函数（分配 caller buffer、参数 set-up、状态检查、存储 buffer 指针）；`allocate_var_storage` 已为 witness Hash 调用分配 32 字节缓冲区；新增 `WitnessMalformed(42)` 和 `WitnessFieldTruncated(43)` 运行时错误；新增三条真实 CKB VM witness 回归覆盖空 WitnessArgs 通过、too-small fail-closed、短 lock BytesOpt 零填充；同时修正 `witness::size` 类型推断和 proof-plan `reads: witness` 元数据。 | 继续补齐 auth 类型化字段、input_type/output_type VM 证据、malformed/truncated/reordered witness 正/负向 VM 测试。 |
...
下一步目标是在保持 `equivalence_status = NOT_PROVEN` 的前提下，把差分
矩阵扩展到 deposit/receipt 形状夹具之外。
...
6. 捕获证据：
   ...
   - 拒绝行失败模式
7. 只有证据真实时才新增或更新矩阵行。
8. 除非所有选定行都满足生产门禁，否则保持 `equivalence_status = NOT_PROVEN`。
...
- 新增 VM 证据：
  ...
  - 夹具 hash
  - 产物 hash
- `equivalence_status` 是否仍为 `NOT_PROVEN`
- gap 文档/矩阵中仍列出的不支持语义
