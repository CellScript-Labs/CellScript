...
- iCKB 被用作高难度、真实 CKB 协议样本，用来检验 CellScript 是否能表达、降低并
  执行 CKB 原生约束。
- 当前 active matrix 已经有 75 条 original-vs-CellScript CKB VM 双侧差分行、
  14 条 CellScript-only VM 行、8 条 original-side VM 行，active `MODEL` 行为 0。
- 生产等价仍然明确标记为 `NOT_PROVEN`，因为真实 owner-auth witness、DAO
  redeem aggregate accounting、通用聚合降低、byte-accurate receipt decoding、
  production evidence manifest closure 和 legacy 非可执行假设清空还没有闭环。
...
- 选定 normalized fixtures 的差分执行证据覆盖已经很高；
- 生产等价准备度仍是部分完成；
- 分支必须保持 `equivalence_status = NOT_PROVEN`；
- 任何生产等价说法都必须先通过 `tests/benchmarks/ickb_diff/matrix.json` 和
  `tests/ickb_diff.rs` 的门禁。
...
当前矩阵故意停在 `PARTIAL_CKB_VM_EXECUTION` / `NOT_PROVEN`。这不是措辞保守，而是
方法论要求：只要还有未执行语义、非可执行模型假设或证据清单缺口，就不能升级。
...
当前不声明 production equivalence，主要因为以下缺口仍然重要：
...
- **byte-accurate receipt decoding**：已有 malformed data 边界证据，但完整 receipt
  字节结构解码还不是通用语言能力。
- **production manifest closure**：binary hash、fixture hash、tx context、capacity、
  fee、deployment/cell dep 证据必须完整闭环后，才能升级 claim。
- **非可执行模型假设清空**：当前 registry 仍登记 3 个 legacy assumption：
  duplicate receipt-id、synthetic wrong-owner resource fields、synthetic current-epoch
  immature redeem。它们已不再作为 active `MODEL` row 计数，但在 production claim 前
  必须清空。
...
3. 如果某个 CellScript helper 只能覆盖一半语义，就只能登记为 precursor evidence，
   不能升级成 differential equivalence。
...
CKB VM 执行、原始脚本差分、证据清单一路走完；已经跑出大量 selected fixture 的
双侧 VM 证据，但只要 owner-auth、DAO redeem accounting、通用聚合降低、receipt
字节解码和 production manifest 还没有闭环，就明确保持 `NOT_PROVEN`，不把
benchmark 进展说成生产等价。
