...
| Capability vocabulary reset | Implemented | Strict mode (`--primitive-strict=0.15`) rejected `has transfer` and `has destroy` with diagnostic CS0150/CS0151. Compatibility mode (`--primitive-compat=0.14`) accepted legacy vocabulary. As of 0.19, `transfer` is removed from the public grammar entirely; `has transfer` and `transfer X to Y` now fail at parse time, not via CS0150. |
