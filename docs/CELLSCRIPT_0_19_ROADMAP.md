...
| Grammar and syntax governance | Done for this slice | Governance docs now include a 0.19 release status matrix; syntax-combo quick/ci/deep emit machine-readable governance and known-bug-class coverage; VS Code grammar/snippets are aligned with `verification`. Legacy `transfer` keyword, `Capability::Transfer`, `Expr::Transfer`, and `TransferExpr` removed from public grammar; `has transfer` and `transfer X to Y` fail at parse time; `std::lifecycle::transfer(...)` remains the canonical transfer pattern. `assert` keyword removed from action/lock context; only `assert_invariant` retained for invariant declarations. |
