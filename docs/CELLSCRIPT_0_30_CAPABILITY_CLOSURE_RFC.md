...
| Typed CKB transaction views and runtime adapters | [#24](https://github.com/CellScript-Labs/CellScript/issues/24) | The bounded v1 candidate unifies admitted Cell/input/output/CellDep/header/witness/Script/hash/source operations without a raw syscall escape hatch. Typed provenance, CKB-VM adversarial coverage, and four exact non-cryptographic resource profiles cover the complete typed read-only handle table: transaction/header/temporal, Output/GroupOutput/Script, persistent policy, and Input/GroupInput/CellDep/WitnessArgs. The independent checker now also closes the exact `InputView<T>` to internal OutPoint-reference boundary. The persistent order policy binds `GroupInput.data_size`/`capacity` through policy dispatch, exact generated-builder/ProtocolBundle materialization, same-transaction composition, and stateful CKB-VM execution; its separate exact resource profile pins every action transaction and maximum resources. Full-header hashing and an in-language multisig-prefix signing transform are excluded from the frozen corpus. Persistent-policy/generated-builder parity for the remaining admitted rows, independent review, and release evidence remain open. |
