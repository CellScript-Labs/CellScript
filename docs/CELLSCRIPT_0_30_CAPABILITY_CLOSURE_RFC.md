...
| Product, publication, and deployment closure | [#27](https://github.com/CellScript-Labs/CellScript/issues/27) | The release boundary remains open and prevents compiler-only evidence or stale generated products from being presented as complete. The maintainer re-accepted retrievable release-evidence pins for VS Code (`4df04807`), website (`4d1acbed`), NovaSeal (`2db4af38`), and iCKB (`53c5078a`), while retaining DOB at `30709c97`. The canonical website compiler was rebuilt twice to the identical SHA-256 `82e76c029af7647fa206684e023fc95abdfd8e15ea6ce12e8993edb75da91b3b` and remains within the 600 KB gzip budget. The editor packaging graph now has zero reported npm vulnerabilities; the website build graph has no moderate/high/critical advisories, with twenty low-severity `elliptic`-chain findings retained because npm's proposed remediation downgrades the maintained CKB connector from 1.3.0 to 0.0.4. Local production admission passed against clean commit `6405ffe0` on pinned CKB `f7fa4436`, and independent review is explicitly waived; exact unified release replay, selected-network deployment, and publication remain pending. |
