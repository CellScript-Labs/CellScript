...
| Typed roles across Script boundaries | [#10](https://github.com/CellScript-Labs/CellScript/issues/10) | Closed artifact-known Cell/witness roles are implemented in ProtocolBundle with exact schema/interface/ELF/deployment identity. The bounded 0.30 scope is closed; runtime-selected open roles are explicitly deferred to [#29](https://github.com/CellScript-Labs/CellScript/issues/29) on top of #28's compatible-handle contract. |
| Runtime Script and verifier handles | [#11](https://github.com/CellScript-Labs/CellScript/issues/11) | Exact artifact receipts and fixed-width values are implemented for independently checked ProtocolBundle artifacts. Canonical checked Script construction/hashing is implemented for fixed args up to 459 bytes. The bounded 0.30 scope is closed; generic compatible-open handles are explicitly deferred to [#28](https://github.com/CellScript-Labs/CellScript/issues/28). |
